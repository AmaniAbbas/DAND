...
with the amount being added for each new row, and a second with the running total.
```
...
```
create and use an alias to shorten the following query that has multiple window
functions. Name the alias account_year_window.
```
SELECT id,
       account_id,
       DATE_TRUNC('year',occurred_at) AS year,
...
levels in terms of the amount of total_amt_usd for their orders.
Your resulting table should have the account_id, the occurred_at time for
